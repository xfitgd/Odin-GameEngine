...
WEBP_MUX_ABI_VERSION :: 0x0109        // MAJOR(8b) + MINOR(8b)

// Error codes
WebPMuxError :: enum c.int {
	OK               = 1,
	NOT_FOUND        = 0,
	INVALID_ARGUMENT = -1,
	BAD_DATA         = -2,
...
WebPChunkId :: enum c.int {
	VP8X,       // VP8X
	ICCP,       // ICCP
	ANIM,       // ANIM
	ANMF,       // ANMF
	DEPRECATED, // (deprecated from FRGM)
	ALPHA,      // ALPH
	IMAGE,      // VP8/VP8L
	EXIF,       // EXIF
	XMP,        // XMP
	UNKNOWN,    // Other chunks.
	NIL,
}

// Encapsulates data about a single frame.
WebPMuxFrameInfo :: struct {
	bitstream:      WebPData,           // image data: can be a raw VP8/VP8L bitstream
	...
	id:             WebPChunkId,        // frame type: should be one of WEBP_CHUNK_ANMF
                          // or WEBP_CHUNK_IMAGE
...
WebPAnimEncoderOptions :: struct {
	anim_params:   WebPMuxAnimParams, // Animation parameters.
...
foreign lib {
	...
	WebPGetMuxVersion :: proc() -> i32 ---

	// Internal, version-checked, entry point
	WebPNewInternal :: proc(i32) -> ^WebPMux ---

	// Creates an empty mux object.
	// Returns:
	//   A pointer to the newly created empty mux object.
	//   Or NULL in case of memory error.
	WebPMuxNew :: proc() -> ^WebPMux ---

	// Deletes the mux object.
	// Parameters:
	//   mux - (in/out) object to be deleted
	WebPMuxDelete :: proc(mux: ^WebPMux) ---

	// Internal, version-checked, entry point
	WebPMuxCreateInternal :: proc(^WebPData, i32, i32) -> ^WebPMux ---
	...
	WebPMuxCreate :: proc(bitstream: ^WebPData, copy_data: i32) -> ^WebPMux ---
	...
	WebPMuxSetChunk :: proc(mux: ^WebPMux, fourcc: cstring, chunk_data: ^WebPData, copy_data: i32) -> WebPMuxError ---
	...
	//   WEBP_MUX_OK - on success.
	WebPMuxGetChunk :: proc(mux: ^WebPMux, fourcc: cstring, chunk_data: ^WebPData) -> WebPMuxError ---
	...
	//   WEBP_MUX_OK - on success.
	WebPMuxDeleteChunk :: proc(mux: ^WebPMux, fourcc: cstring) -> WebPMuxError ---
	...
	//   WEBP_MUX_MEMORY_ERROR - on memory allocation error.
	//   WEBP_MUX_OK - on success.
	WebPMuxSetImage :: proc(mux: ^WebPMux, bitstream: ^WebPData, copy_data: i32) -> WebPMuxError ---
	...
	//   WEBP_MUX_MEMORY_ERROR - on memory allocation error.
	//   WEBP_MUX_OK - on success.
	WebPMuxPushFrame :: proc(mux: ^WebPMux, frame: ^WebPMuxFrameInfo, copy_data: i32) -> WebPMuxError ---
	...
	//   WEBP_MUX_MEMORY_ERROR - on memory allocation error.
	//   WEBP_MUX_OK - on success.
	WebPMuxGetFrame :: proc(mux: ^WebPMux, nth: u32, frame: ^WebPMuxFrameInfo) -> WebPMuxError ---
	...
	//                        before deletion.
	//   WEBP_MUX_OK - on success.
	WebPMuxDeleteFrame :: proc(mux: ^WebPMux, nth: u32) -> WebPMuxError ---
	...
	//   WEBP_MUX_MEMORY_ERROR - on memory allocation error.
	//   WEBP_MUX_OK - on success.
	WebPMuxSetAnimationParams :: proc(mux: ^WebPMux, params: ^WebPMuxAnimParams) -> WebPMuxError ---
	...
	//   WEBP_MUX_OK - on success.
	WebPMuxGetAnimationParams :: proc(mux: ^WebPMux, params: ^WebPMuxAnimParams) -> WebPMuxError ---
	...
	//   WEBP_MUX_OK - on success.
	WebPMuxSetCanvasSize :: proc(mux: ^WebPMux, width: i32, height: i32) -> WebPMuxError ---
	...
	//   WEBP_MUX_OK - on success.
	WebPMuxGetCanvasSize :: proc(mux: ^WebPMux, width: ^i32, height: ^i32) -> WebPMuxError ---
	...
	//   WEBP_MUX_BAD_DATA - if VP8X/VP8/VP8L chunk or canvas size is invalid.
	//   WEBP_MUX_OK - on success.
	WebPMuxGetFeatures :: proc(mux: ^WebPMux, flags: ^u32) -> WebPMuxError ---
	...
	//   WEBP_MUX_OK - on success.
	WebPMuxNumChunks :: proc(mux: ^WebPMux, id: WebPChunkId, num_elements: ^i32) -> WebPMuxError ---
	...
	//   WEBP_MUX_MEMORY_ERROR - on memory allocation error.
	//   WEBP_MUX_OK - on success.
	WebPMuxAssemble :: proc(mux: ^WebPMux, assembled_data: ^WebPData) -> WebPMuxError ---

	// Internal, version-checked, entry point.
	WebPAnimEncoderOptionsInitInternal :: proc(^WebPAnimEncoderOptions, i32) -> i32 ---

	// Should always be called, to initialize a fresh WebPAnimEncoderOptions
	// structure before modification. Returns false in case of version mismatch.
	// WebPAnimEncoderOptionsInit() must have succeeded before using the
	// 'enc_options' object.
	WebPAnimEncoderOptionsInit :: proc(enc_options: ^WebPAnimEncoderOptions) -> i32 ---

	// Internal, version-checked, entry point.
	WebPAnimEncoderNewInternal :: proc(i32, i32, ^WebPAnimEncoderOptions, i32) -> ^WebPAnimEncoder ---
	...
	//   Or NULL in case of memory error.
	WebPAnimEncoderNew :: proc(width: i32, height: i32, enc_options: ^WebPAnimEncoderOptions) -> ^WebPAnimEncoder ---
	...
	WebPAnimEncoderAdd :: proc(enc: ^WebPAnimEncoder, frame: ^WebPPicture, timestamp_ms: i32, config: ^WebPConfig) -> i32 ---
	...
	// Returns:
	//   True on success.
	WebPAnimEncoderAssemble :: proc(enc: ^WebPAnimEncoder, webp_data: ^WebPData) -> i32 ---
	...
	WebPAnimEncoderGetError :: proc(enc: ^WebPAnimEncoder) -> cstring ---

	// Deletes the WebPAnimEncoder object.
	// Parameters:
	//   enc - (in/out) object to be deleted
	WebPAnimEncoderDelete :: proc(enc: ^WebPAnimEncoder) ---
	...
	//   WEBP_MUX_MEMORY_ERROR - on memory allocation error.
	//   WEBP_MUX_OK - on success.
	WebPAnimEncoderSetChunk :: proc(enc: ^WebPAnimEncoder, fourcc: cstring, chunk_data: ^WebPData, copy_data: i32) -> WebPMuxError ---
	...
	//   WEBP_MUX_OK - on success.
	WebPAnimEncoderGetChunk :: proc(enc: ^WebPAnimEncoder, fourcc: cstring, chunk_data: ^WebPData) -> WebPMuxError ---
	...
	//   WEBP_MUX_OK - on success.
	WebPAnimEncoderDeleteChunk :: proc(enc: ^WebPAnimEncoder, fourcc: cstring) -> WebPMuxError ---
}
